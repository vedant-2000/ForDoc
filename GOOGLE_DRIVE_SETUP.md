...
1. **Redirect URIs must be `https://…` or `http://localhost`.**
   A plain-HTTP IP address like `https://treatmentrecord.duckdns.org` **cannot** be
...
- **Authorised redirect URIs → Add URI**, exactly this, with no trailing
  slash:
  ```
  https://treatmentrecord.duckdns.org/api/drive/callback
  ```
...
GOOGLE_REDIRECT_URI=https://treatmentrecord.duckdns.org/api/drive/callback
```
...
3. The browser now tries to reach `https://treatmentrecord.duckdns.org/…` and shows
...
4. **Copy the entire address** from the browser's address bar. It looks like:
   ```
   https://treatmentrecord.duckdns.org/api/drive/callback?code=4/0AVMBsJ...&scope=https://www.googleapis.com/auth/drive
   ```
